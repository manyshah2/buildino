...
retry_cmd(){ local label="$1";shift;local rc=1;for a in 1 2 3;do local log="handoff/logs/${label}-attempt${a}.log";set +e;(cd "$project_dir"&&"$@") 2>&1|tee "$log";rc=${PIPESTATUS[0]};set -e;[ $rc -eq 0 ]&&return 0;if [ $a -lt 3 ]&&transient "$log";then sleep $((a*8));else return $rc;fi;done;return $rc; }
